...
interface SkillItem {
  ...
  name: string;
  image: string;
  description: { es: ReactNode; en: ReactNode };
}
...
export const Skills = () => {
  ...
  const [lastPage, setLastPage] = useState<{ index: number; direction: Direction }>({ index: 1, direction: 'right' });

  useEffect(() => {
    const detDirection = (c: { index: number; direction: Direction }): Direction => {
    ...
    <div id="skill" onClick={onFocus}>
      <div id="carouselExampleIndicators" className="carousel slide" data-bs-ride="carousel">
        ...
        <Body className="carousel-inner">
          {skills.map(({ name, id, image, description }) => <div
            key={name}
            className={id === currentPage ? "carousel-item active" : "carousel-item"}
          >
            <ShowSkillData direction={lastPage.direction} id={id} currentPage={currentPage} name={name} image={image}> {language === 'es' ? description.es : description.en} </ShowSkillData></div>)}
        </Body>
